...
const AuthModule = ({ isUpdatePassword = false }) => {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      setMessage("¡Tu contraseña ha sido actualizada correctamente! Redirigiendo...");
      setTimeout(() => {
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div style={{ 
            ...
            {isUpdatePassword ? <Lock size={32} /> : (isResetMode ? <KeyRound size={32} /> : (isLogin ? <LogIn size={32} /> : <UserPlus size={32} />))}
          </div>
          <h2 style={{ fontSize: '1.8rem', fontWeight: 900, color: '#0f172a', margin: 0, letterSpacing: '-0.5px' }}>
            {isUpdatePassword ? 'Nueva Contraseña' : (isResetMode ? 'Recuperar Contraseña' : (isLogin ? 'Acceso Corporativo' : 'Crear Cuenta'))}
          </h2>
          <p style={{ color: '#64748b', fontSize: '0.9rem', marginTop: '8px' }}>
            {isUpdatePassword ? 'Ingresa tu nueva contraseña para acceder al sistema' : 'SaaS Rutas - Plataforma de Inteligencia Logística'}
          </p>
        </div>
        ...
        {isUpdatePassword ? (
          <form onSubmit={handleUpdatePassword} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, color: '#475569', marginBottom: '8px' }}>NUEVA CONTRASEÑA</label>
            ...
            >
              {loading ? <div className="loader-dots"></div> : 'Guardar Nueva Contraseña'}
            </button>
          </form>
        ) : (
          <>

...
const mapMock = { setView: () => mapMock, invalidateSize: () => {}, on: () => {} };
global.L = {
    map: () => mapMock,
    tileLayer: () => ({ addTo: ()=>{} }), divIcon: () => ({}),
    control: { zoom: () => ({ addTo: ()=>{} }), layers: () => ({ addTo: ()=>{} }) },
    marker: () => markerMock, polyline: () => ({ addTo: ()=>{} })
};
global.document = {
    body: { classList: { add: ()=>{}, remove: ()=>{} } },
    getElementById: () => ({ innerText: '', innerHTML: '', style: { setProperty: () => {} }, classList: { add: ()=>{}, remove: ()=>{} }, appendChild: ()=>{}, addEventListener: ()=>{} }),
    createElement: () => ({ innerText: '', innerHTML: '', style: { setProperty: () => {} }, classList: { add: ()=>{}, remove: ()=>{} }, appendChild: ()=>{}, addEventListener: ()=>{} }),
    querySelectorAll: () => []
};
...
function runDataset(name, jsonPath) {
    console.log('================================================================');
    ...
    const solved = solveHybridLogisticsPlansJS(jobs);
    let earlyArrivals = 0;
    let lateArrivals = 0;
    let citasHonored = 0;

    solved.plan1.routes.forEach(r => {
        console.log(`\n[${r.emb_id}] ${r.vehicle} (${r.type}) | ${r.corridor} | Carga: ${(r.load/1000).toFixed(2)}t | Salida: ${r.departure} | Retorno: ${r.return_time}`);
        r.stops.forEach(s => {
            console.log(`   Parada ${s.seq}: ${s.city} (${s.state}) [${(s.load/1000).toFixed(2)}t] -> ETA: ${s.eta} | ${s.windowStatus} ${s.cita ? '| Cita: "' + s.cita + '"' : ''}`);
            if (s.windowStatus === 'CITA_PUNTUAL') citasHonored++;
        });
    });

    console.log(`\nTotal Routes: ${solved.plan1.routes.length} | Citas Honored: ${citasHonored}`);
}

runDataset('EJERCICIO PYTHON 2.xlsx', 'scratch/test_sap_rows.json');
runDataset('PRUEBA a.xlsx', 'scratch/prueba_a_rows.json');
